    ...
    int B_cols)
{
    ...
    // i: A matrisinin satırlarını gezer
    for (i = 0; i < A_rows; ++i) {

        // j: B matrisinin sütunlarını gezer
        for (j = 0; j < B_cols; ++j) {

            // k: Ortak boyutu gezer (A'nın sütunu, B'nin satırı)
            // Matris çarpımı formülü: Satır x Sütun
            for (k = 0; k < A_cols; ++k) {
                // A[i][k] * B[k][j] işleminin tek boyutlu dizi hali:
                int a_val = A[i * A_cols + k];
                int b_val = B[k * B_cols + j];

                C[i * B_cols + j] += a_val * b_val;
            }
